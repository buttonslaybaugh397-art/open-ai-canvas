...
CADDYFILE_URL="${CADDYFILE_URL:-https://raw.githubusercontent.com/ddcat-ai/open-ai-canvas/${REPOSITORY_REF}/Caddyfile.example}"
...
download_caddyfile() {
    ...
    step "下载 Caddy 反向代理模板"
    local temporary_file
    temporary_file="$(mktemp "${INSTALL_DIR}/.Caddyfile.example.XXXXXX")"
    curl -fsSL "$CADDYFILE_URL" -o "$temporary_file"
    mv "$temporary_file" Caddyfile.example
}
