...
import { ArrowDownLeft, ArrowUpRight, BarChart3, CalendarCheck, CalendarDays, Coins, CreditCard, RefreshCw, RotateCcw, ShieldCheck, SlidersHorizontal, Sparkles, TicketCheck, TrendingUp } from "lucide-react";
...
export default function WalletPage() {
    ...
    const consumptionDaily = useMemo(
        () =>
            (wallet?.consumption?.daily || []).map((item) => ({
                ...item,
                credits: item.amountMicrocredits / 1_000_000,
            })),
        [wallet?.consumption?.daily],
    );
    const hasConsumption = consumptionDaily.some((item) => item.amountMicrocredits > 0);
    ...
    return (
        <main className="app-user-content app-workspace-scroll library-page wallet-library-page wallet-market-page relative h-full overflow-y-auto text-foreground">
            <div className="relative w-full px-4 py-6 sm:px-6 lg:px-8">
                ...
                <section className="wallet-consumption-panel app-workspace-surface mt-6 rounded-lg p-5 sm:p-6" aria-labelledby="wallet-consumption-title">
                    <div className="wallet-consumption-heading">
                        <div className="flex min-w-0 items-start gap-3">
                            <span className="wallet-consumption-heading-icon">
                                <BarChart3 className="size-4" />
                            </span>
                            <div className="min-w-0">
                                <h2 id="wallet-consumption-title" className="text-base font-semibold">个人消耗</h2>
                                <p className="mt-1 text-xs leading-5 text-foreground/55">按上海时间统计已结算的模型消费，冻结和退款不计入消耗。</p>
                            </div>
                        </div>
                        <span className="wallet-consumption-period">本月每日趋势</span>
                    </div>

                    {!wallet && loading ? (
                        <Skeleton className="mt-5" active paragraph={{ rows: 5 }} />
                    ) : (
                        <>
                            <div className="wallet-consumption-metrics">
                                <ConsumptionMetric label="今日消耗" description="今日 00:00 至现在" value={wallet?.consumption?.todayMicrocredits || 0} icon={<CalendarCheck />} />
                                <ConsumptionMetric label="昨日消耗" description="昨日 00:00 - 24:00" value={wallet?.consumption?.yesterdayMicrocredits || 0} icon={<CalendarDays />} />
                                <ConsumptionMetric label="本周消耗" description="本周一至今日" value={wallet?.consumption?.weekMicrocredits || 0} icon={<TrendingUp />} />
                                <ConsumptionMetric label="本月消耗" description="本月一日至今日" value={wallet?.consumption?.monthMicrocredits || 0} icon={<Coins />} />
                            </div>
                            {hasConsumption ? (
                                <div className="wallet-consumption-chart" role="img" aria-label="本月个人每日积分消耗图">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <BarChart data={consumptionDaily} margin={{ top: 10, right: 4, bottom: 0, left: -12 }}>
                                            <CartesianGrid stroke="currentColor" className="text-foreground/10" vertical={false} />
                                            <XAxis dataKey="day" tickFormatter={(value) => String(value).slice(5)} tick={{ fontSize: 11 }} axisLine={false} tickLine={false} minTickGap={18} />
                                            <YAxis tickFormatter={formatChartCredits} tick={{ fontSize: 11 }} axisLine={false} tickLine={false} width={54} />
                                            <ChartTooltip
                                                cursor={{ fill: "var(--muted)", fillOpacity: 0.45 }}
                                                labelFormatter={(value) => formatConsumptionDay(String(value))}
                                                formatter={(value, _name, item) => [`${formatCredits(Number(item.payload.amountMicrocredits), 6)} 积分 · ${item.payload.count} 次`, "已结算消耗"]}
                                            />
                                            <Bar dataKey="credits" name="已结算消耗" fill="var(--chart-2)" radius={[3, 3, 0, 0]} maxBarSize={26} />
                                        </BarChart>
                                    </ResponsiveContainer>
                                </div>
                            ) : (
                                <div className="wallet-consumption-empty">
                                    <BarChart3 className="size-5" />
                                    <div>本月暂无已结算消耗</div>
                                    <p>完成一次模型生成后，每日用量会在这里按日期累计。</p>
                                </div>
                            )}
                        </>
                    )}
...
function ConsumptionMetric({ label, description, value, icon }: { label: string; description: string; value: number; icon: ReactNode }) {
    return (
...
function formatConsumptionDay(value: string) {
    const [year, month, day] = value.split("-");
    return `${year} 年 ${Number(month)} 月 ${Number(day)} 日`;
}

function formatChartCredits(value: number) {
    if (value >= 1000) return `${(value / 1000).toFixed(value >= 10_000 ? 0 : 1)}k`;
    if (value >= 1) return value.toLocaleString("zh-CN", { maximumFractionDigits: 1 });
