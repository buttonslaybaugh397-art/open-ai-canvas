        ...
        const applyTasks = async (tasks: GenerationTask[]) => {
            ...
            for (const task of attachable) {
                await consumeGenerationTaskMessage(
                    ...
                    async ({ effectKey, resultUrls, resultSources }) => {
                        if (cancelled) return;
                        await updateConversationMessage(
                            ...
                            (item) =>
                                applyGenerationConsumerEffect(item, effectKey, (current) => ({
                                    ...
                                    resultSources: mergeCreationMediaSources(current.resultSources, resultSources),
                                })).value,
                        );
                    },
            ...
            if (mode === "text") {
                ...
                const storedImages = await Promise.allSettled(
                    generatedImages.map(async ({ image, taskId, batchIndex }) => {
                        ...
                        const materialized = await consumeGenerationTaskMessage(
                            task,
                            assistantMessage.id,
                            async ({ resultUrls, resultSources, effectKey }) => {
                ...
                const materialized = await consumeGenerationTaskMessage(
                    ...
                    async ({ resultUrls, resultSources, effectKey }) => {
                        await updateOriginAssistant(
                            (item) =>
                                applyGenerationConsumerEffect(item, effectKey, (current) => ({ ...current, status: "done" as const, content: "视频已生成", resultUrls, resultSources: mergeCreationMediaSources(current.resultSources, resultSources) }))
                                    .value,
                        );
                    },
                    { signal: requestLifecycle.signal },
        ...
        >
            <div className="creation-history-content">
                <label className="creation-history-search">
                    ...
                    <input name="conversation-search" value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="搜索对话标题或内容" aria-label="搜索历史对话" />
                </label>
                ...
                <div className="creation-user-message-attachments">
                    {item.attachments.map((attachment) => {
                        ...
                        return (
                            ...
                            >
                                {kind === "video" ? (
                                    <video src={attachment.url || ""} poster={url !== attachment.url ? url : undefined} muted playsInline preload="none" />
                                ) : kind === "image" ? (
    ...
    return (
        <div className="creation-media-result">
            {isVideo && videoSource ? (
                <button
                    ...
                    onClick={() => {
                        ...
                        setPreviewUrl(videoSource.url);
                    }}
                    aria-label="预览生成视频"
                >
                    <MediaPreview src={videoSource.url} kind="video" fallbackStorageKey={videoSource.storageKey} className="size-full object-cover" />
                    <span>
                        <Maximize2 />
                        预览视频
                    </span>
            ...
            <div className="creation-media-actions">
                ...
                {isVideo && videoSource ? (
                    <CreationMediaDownloadLink source={videoSource} />
                ) : (
                    resultUrls.map((url, index) => <CreationMediaDownloadLink key={`${url}-download`} source={creationMediaSourceForUrl(resultSources, url)} fileName={resultUrls.length > 1 ? `生成图片-${index + 1}.png` : "生成图片.png"} />)
                )}
            </div>
                ...
                return (
                    <span key={reference.id} className="creation-user-message-reference">
                        {reference.previewUrl && reference.kind === "video" ? (
                            <video src={reference.previewUrl} muted playsInline preload="none" aria-label={reference.label} />
...
function CreationMediaPreviewModal({ url, type, source, onClose }: { url: string; type: "image" | "video"; source?: TaskMediaSource; onClose: () => void }) {
    return (
        <Modal
            ...
            styles={{ body: { padding: 0 } }}
        >
            {url ? <MediaPreview src={source?.url || url} kind={type} fallbackStorageKey={source?.storageKey} controls={type === "video"} className={type === "video" ? "creation-media-preview-video" : "creation-media-preview-image"} /> : null}
        </Modal>
    );
}
...
function CreationAttachmentThumbnail({ item, onPreview, onRemove }: { item: CreationAttachment; onPreview: (type: "image" | "video", url: string) => void; onRemove: (id: string) => void }) {
    ...
    const content =
        kind === "video" ? (
            <video src={item.url} poster={item.previewUrl !== item.url ? item.previewUrl : undefined} muted playsInline preload="none" aria-label={item.name} />
        ) : kind === "image" ? (
    ...
    const composer = (
        <section className={`creation-chat-composer is-${props.variant}`}>
            <div className="creation-chat-writing-surface">
                <input ref={props.fileInputRef} name="creation-upload" type="file" hidden accept={creationUploadAccept(props.mode)} multiple onChange={props.onFileChange} />
                        ...
                        {props.mode !== "video" &&
                            ...
                            (customRatioOpen ? (
                                <label className="creation-custom-value">
                                    <span>宽 x 高</span>
                                    <input
                                        name="image-custom-ratio"
                                        value={props.ratio}
                                        onFocus={(event) => event.currentTarget.select()}
                                        onChange={(event) => props.setRatio(event.target.value)}
                                        placeholder="1920x1080 或 2:1"
                                        aria-label="自定义图片尺寸或比例"
                                    />
                                </label>
                            ) : (
        ...
        profile.duration.selection === "range" ? (
            <>
                <input
                    ...
                    style={{ accentColor: "var(--creation-text)" }}
                    type="range"
                    min={min}
                    max={max}
                    step={step}
                                    ...
                                    <li key={shot.user?.id || shot.result?.id || index}>
                                        ...
                                        >
                                            <span className="storyboard-workbench-rail-thumb">
                                                {thumbUrl ? (
                                                    thumbIsVideo ? (
                                                        <video muted preload="none" src={thumbUrl} />
                                                    ) : (
        ...
        <article className={`storyboard-workbench-card is-${status}`}>
            <header className="storyboard-workbench-card-head">
                ...
                <div className="storyboard-workbench-card-actions">
                    ...
                    {mode === "video" && resultSources[0] ? (
                        <CreationMediaDownloadLink source={resultSources[0]} />
                    ) : (
                        resultUrls.map((url, index) => <CreationMediaDownloadLink key={`${url}-download`} source={creationMediaSourceForUrl(resultSources, url)} fileName={resultUrls.length > 1 ? `生成图片-${index + 1}.png` : "生成图片.png"} />)
                    )}
                </div>
    ...
    return (
        <>
            {mode === "video" && videoSource ? (
                <button type="button" className="creation-video-result" onClick={() => openPreview(videoSource.url, "video")} aria-label="预览生成视频">
                    <MediaPreview src={videoSource.url} kind="video" fallbackStorageKey={videoSource.storageKey} className="size-full object-cover" />
                    <span>
                        <Maximize2 />
                        预览视频
...
function reconcileCreationTaskMessages(conversations: CreationConversation[], tasks: PersistedCreationTask[]) {
    ...
    const next = conversations.map((conversation) => {
        ...
        const messages = conversation.messages.map((message) => {
            ...
            const resultSources = mergeCreationMediaSources(
                undefined,
                matches.filter((task) => task.status === "succeeded").flatMap((task) => task.creationResultSources || []),
            );
