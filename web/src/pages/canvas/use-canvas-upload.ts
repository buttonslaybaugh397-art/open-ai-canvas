...
export function useCanvasUpload({ canvasId, domainProjectId, nodesRef, selectedNodeIdsRef, getCanvasCenter, screenToCanvas, setNodes, setSelectedNodeIds, setSelectedConnectionId, setContextMenu, setDialogNodeId }: UseCanvasUploadOptions) {
    ...
    const createFileNode = useCallback(
        async (file: File, position: Position, replaceId?: string) => {
            ...
            try {
                ...
                if (placeholder.type === CanvasNodeType.Image) {
                    metadata = { ...imageMetadata(await uploadImage(file, onProgress)), fileName: file.name };
                    ...
                    metadata = { content, prompt: content, fileName: file.name, storageKey: resourceStorageKey(resource.id), mimeType: file.type || "text/plain", bytes: file.size, status: "success" };
                } else {
                    const media = await uploadMediaFile(file, placeholder.type === CanvasNodeType.Video ? "video" : "audio", onProgress);
                    metadata = { ...(placeholder.type === CanvasNodeType.Video ? videoMetadata(media) : audioMetadata(media)), fileName: file.name };
                }
                    ...
                    metadata: {
                        ...
                        ...(replaceId
                            ? {
                                  ...
                                  generationResultPlacement: undefined,
                                  copiedFromNodeId: undefined,
        ...
        async (asset: ImageAsset, position?: Position) => {
            try {
                ...
                const node: CanvasNodeData = {
                    ...
                    metadata: {
                        content,
                        fileName: typeof asset.metadata?.fileName === "string" && asset.metadata.fileName.trim() ? asset.metadata.fileName.trim() : asset.title,
        ...
        async (event: ChangeEvent<HTMLInputElement>) => {
            ...
            try {
                ...
                if (target?.nodeId) {
                    ...
                    const compatible = targetNode && (targetNode.type === uploadNodeType(file) || ![CanvasNodeType.Image, CanvasNodeType.Video, CanvasNodeType.Audio, CanvasNodeType.Text].includes(targetNode.type as CanvasNodeType));
                    if (!compatible) {
    ...
    const createAssetPayloadNode = useCallback(async (payload: InsertAssetPayload, center: Position) => {
        ...
        if (payload.kind === "audio") {
            ...
            return {
                ...
                width: spec.width,
                height: spec.height,
                metadata: { content: payload.url, fileName: payload.title, storageKey: payload.storageKey, durationMs: payload.durationMs, bytes: payload.bytes, mimeType: payload.mimeType || "audio/mpeg", assetId: payload.assetId, status: NODE_STATUS_SUCCESS },
            } satisfies CanvasNodeData;
        }
        ...
        const metadata = storedImage.storageKey
            ? { ...imageMetadata({ ...storedImage, storageKey: storedImage.storageKey, width: meta.width, height: meta.height }), fileName: payload.title }
            : { content: storedImage.url, fileName: payload.title, status: NODE_STATUS_SUCCESS, naturalWidth: meta.width, naturalHeight: meta.height, bytes: storedImage.bytes, mimeType: storedImage.mimeType };
        return {
            id,
            type: CanvasNodeType.Image,
