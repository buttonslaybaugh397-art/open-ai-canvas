...
function syncOverlayContent(scene: OverlayScene, props: CanvasLeaferGraphicsLayerProps, viewportScale: number) {
    ...
    if (connecting) {
        scene.draft.set({
            path: activeConnectionPath(
                ...
                props.connectionTargetHandleId,
    ...
    if (!batch) return;
    const target = batch.snapToHandle && batch.targetNodeId ? props.nodeById.get(batch.targetNodeId) : undefined;
    ...
    batch.sourceNodeIds.forEach((sourceNodeId) => {
        ...
        scene.batchDrafts.add(new Path({
            path: activeConnectionPath(source, handle, batch.mouseWorld, target, props.scriptScrollTopById[source.id] || 0, batch.targetHandleId, target ? props.scriptScrollTopById[target.id] || 0 : 0),
            stroke,
            strokeWidth: 1.4,
            strokeScaleFixed: true,
            strokeCap: "round",
