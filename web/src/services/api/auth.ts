...
export type AdminAnalytics = {
    ...
    kpi: {
        ...
        cachedTokens: number;
        usageAvailable: boolean;
        creditsConsumedMicrocredits: number;
...
export type AdminOSSSetting = {
    ...
    credentialsRequireReset: boolean;
    pathStyle: boolean;
    allowUserS3: boolean;
...
export type AdminOSSCredentials = {
    provider: OSSProvider;
    accessKeyId: string;
    accessKeySecret: string;
    sessionToken: string;
...
export function updateAdminUser(id: string, input: Partial<Pick<LocalUser, "username" | "displayName" | "email" | "role" | "status">> & { password?: string }) {
...
export function getAdminOSSCredentials() {
    return http.get<{ credentials: AdminOSSCredentials }>("/admin/settings/oss/credentials");
}
