...
function fixture(overrides = {}) {
    ...
    return {
        run: (script, ...args) => {
            const source = readFileSync(path.join(scripts, script), "utf8").replace(/\r\n/g, "\n");
            const result = spawnSync(bash, ["--noprofile", "--norc", "-s", "--", ...args], { env, encoding: "utf8", input: 'export PATH="$MOCK_BIN:$PATH"\n' + source });
