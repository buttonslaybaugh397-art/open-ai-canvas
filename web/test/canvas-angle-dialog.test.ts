...
const modalSource = readFileSync(new URL("../src/components/ui/product/app-modal/app-modal.tsx", import.meta.url), "utf8");
...
describe("image angle editor (#432)", () => {
    test("uses a centered dismissible modal instead of a node-anchored overlay", () => {
        ...
        expect(modalSource).toContain("destroyOnHidden = true");
        expect(modalSource).toContain("destroyOnHidden={destroyOnHidden}");
