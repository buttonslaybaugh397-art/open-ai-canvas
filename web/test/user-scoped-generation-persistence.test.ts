...
test("creation recovery observes streaming text tasks after reload", () => {
    const conversations = [{
        ...
        messages: [
            { id: "text-streaming", role: "assistant" as const, mode: "text", status: "streaming", taskIds: ["task-text"] },
            { id: "text-done", role: "assistant" as const, mode: "text", status: "done", taskIds: ["task-text-done"] },
            { id: "text-unbound", role: "assistant" as const, mode: "text", status: "streaming", taskIds: [] },
            { id: "image-pending", role: "assistant" as const, mode: "image", status: "pending", taskIds: ["task-image"] },
        ],
    }];
...
// 浏览器中途关闭后任务永远停在 text_replay：worker 不接管它，
// 恢复时必须交还已存草稿并说明中断，不能谎报“生成失败”。
test("creation recovery returns saved draft for interrupted text replay tasks", () => {
    const message = { mode: "text", status: "streaming", content: "", taskIds: ["task-text"] };
    const baseTask = {
        id: "task-text",
        type: "text",
        ...
        createdAt: "2026-08-19T22:47:51.000Z",
        updatedAt: "2026-08-19T22:48:39.000Z",
        status: "text_replay" as const,
    };

    expect(recoverCreationTextTask(message, [{ ...baseTask, textDraft: "已生成的前半段" }])).toEqual({
        status: "error",
        content: "已生成的前半段",
        error: "生成在上次会话中被中断，仅恢复已保存的部分正文",
        taskIds: ["task-text"],
    });
    expect(recoverCreationTextTask(message, [baseTask])).toEqual({
        status: "error",
        content: "生成已中断",
        error: "生成在上次会话中被中断，没有保存到正文",
        taskIds: ["task-text"],
    });
});
