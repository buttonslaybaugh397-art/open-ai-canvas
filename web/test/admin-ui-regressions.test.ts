...
test("model reference limits use compact rows only inside the admin editor", async () => {
    ...
    const grid = sourceSection(css, ".admin-model-editor-references .admin-capability-reference-grid {", ".admin-model-editor-modal .admin-capability-reference-card,");
...
test("request logs display user credit billing independently from upstream cost", async () => {
    ...
    expect(detailSource).toContain('["成本价格（积分）", log.creditCostMicrocredits');
    expect(billingSummary).toContain("creditCostConfigured");
