...
const { resolveGeneratedVideo } = await import("../../src/services/file-storage");
const alreadyAborted = new AbortController();
alreadyAborted.abort();
const beforeAbortReads = reads;
await assert.rejects(resolveGeneratedVideo({ storageKey: "resource:pre-aborted" }, alreadyAborted.signal), { name: "AbortError" });
assert.equal(reads, beforeAbortReads);
const customAbort = new AbortController();
const cancelReason = new Error("consumer disposed");
customAbort.abort(cancelReason);
await assert.rejects(resolveGeneratedVideo({ storageKey: "resource:custom-abort" }, customAbort.signal), (error) => error === cancelReason);

writeGate = new Promise<void>((resolve) => { releaseWrite = resolve; });
const cancelStarted = new Promise<void>((resolve) => { writeStarted = resolve; });
const controller = new AbortController();
const cancelPending = resolveGeneratedVideo({ storageKey: "resource:cancel-persist" }, controller.signal);
const cancelRejected = assert.rejects(cancelPending, { name: "AbortError" });
await cancelStarted;
controller.abort();
releaseWrite();
...
writeGate = undefined;
writeStarted = undefined;
const beforeReuseReads = reads;
const reused = await resolveGeneratedVideo({ storageKey: "resource:cancel-persist", durationMs: 4000 });
assert.equal(reads, beforeReuseReads, "cancellation must not evict the shared persisted cache");
assert.equal(reused.bytes, 5);
assert.equal(reused.mimeType, "video/mp4");
assert.equal(reused.durationMs, 4000);
...
console.log(JSON.stringify({ checks: 17 }));
