...
func TestAdminPasswordResetPreservesAccountAndRevokesOnlyTargetSessions(t *testing.T) {
	...
	if !verifyPassword(password, updated.PasswordHash) || verifyPassword("old-password-123", updated.PasswordHash) {
...
func TestAdminPasswordResetWithoutEmailKeepsDisabledStatus(t *testing.T) {
	...
	if updated.Email != "" || updated.Status != model.UserStatusDisabled || !verifyPassword("recovered-password", updated.PasswordHash) {
