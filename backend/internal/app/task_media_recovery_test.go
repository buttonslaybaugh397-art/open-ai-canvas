...
func TestDownloadTaskMediaResumesTruncatedResponseWithRange(t *testing.T) {
	t.Setenv("CANVAS_ALLOWED_PRIVATE_UPSTREAM_HOSTS", "127.0.0.1")
	s, _ := newMediaRecoveryTestService(t)
	payload := []byte("video-payload-with-a-truncated-first-response")
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case 2:
			...
			w.Header().Set("Content-Length", strconv.Itoa(len(payload)-5))
			w.WriteHeader(http.StatusPartialContent)
			_, _ = w.Write(payload[5:])
		default:
			t.Errorf("unexpected download attempt %d", requests.Load())
		}
	}))
	defer server.Close()

	name, mimeType, err := s.downloadTaskMedia(context.Background(), providerConfig{BaseURL: server.URL}, server.URL+"/video.mp4", "video")
	if err != nil {
		t.Fatal(err)
	}
	defer os.Remove(s.mediaTempPath(name))
	got, err := os.ReadFile(s.mediaTempPath(name))
	if err != nil {
		t.Fatal(err)
	}
	...
	if mimeType != "video/mp4" {
		t.Fatalf("mime type = %q, want video/mp4", mimeType)
	}
	if requests.Load() != 2 {
...
func TestMediaRecoveryDownloadFailureResumesSameTaskAfterRestart(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if downloads.Add(1) <= mediaDownloadAttempts {
	...
	if downloads.Load() != int32(mediaDownloadAttempts+1) {
...
func TestMediaRecoveryRetryDelayStaysShort(t *testing.T) {
	for _, tc := range []struct {
		name    string
		cause   error
		maxWait time.Duration
	}{
		{name: "network", cause: errors.New("connection reset"), maxWait: 9 * time.Second},
		{name: "provider retry after", cause: providerHTTPError{StatusCode: http.StatusTooManyRequests, RetryAfter: 10 * time.Minute}, maxWait: 2*time.Minute + 5*time.Second},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			checkpoint := &mediaCheckpoint{
				Mode:      "image",
				StartedAt: time.Now(),
				Items:     []mediaCheckpointItem{{Reference: protocol.MediaReference{URL: "https://example.test/result"}}},
			}
			if err := s.saveMediaCheckpoint(task, checkpoint, "download"); err != nil {
				t.Fatal(err)
			}
			started := time.Now()
			if err := s.handleMediaRecoveryFailure(task, &mediaRecoveryError{stage: "download", retryable: true, cause: tc.cause}); err != nil {
				t.Fatal(err)
			}
			saved, err := s.repo.Task(task.ID)
			if err != nil {
			...
			if saved.NextPollAt == nil {
				t.Fatal("recovery did not schedule a retry")
			}
			if delay := saved.NextPollAt.Sub(started); delay > tc.maxWait {
				t.Fatalf("retry delay = %s, want less than %s", delay, tc.maxWait)
			}
		})
	}
}
