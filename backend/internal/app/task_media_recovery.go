...
// DownloadTaskMedia already performs short in-request retries. The outer
// recovery loop only needs a brief cooldown for provider throttling or storage
// failures; minute-scale gaps made completed videos appear missing for too long.
var mediaRecoveryDelays = []time.Duration{5 * time.Second, 15 * time.Second, 30 * time.Second, time.Minute}
...
func (s *Service) handleMediaRecoveryFailure(task *model.Task, cause error) error {
	...
	if task.MediaRecoveryJSON != "" {
		...
		if failure.retryable && checkpoint.Attempts < len(mediaRecoveryDelays) && time.Since(checkpoint.StartedAt) < 20*time.Minute {
			delay := mediaRecoveryDelays[checkpoint.Attempts] + time.Duration(rand.IntN(2_000))*time.Millisecond
			var upstream providerHTTPError
			if errors.As(cause, &upstream) {
				delay = max(delay, min(upstream.RetryAfter, 2*time.Minute))
			}
			checkpoint.Attempts++
