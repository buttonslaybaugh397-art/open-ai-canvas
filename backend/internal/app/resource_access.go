...
func (s *Service) resolveResourceAccess(resource *model.Resource, options ResourceAccessOptions) (*ResourceAccess, error) {
	...
	if resource.Provider != "local" {
		...
		setting = restoreLegacyCDNDelivery(setting)
	}
	access, err := assets.ResolveAccess(resource, setting, options, time.Now().UTC(), func(variant assets.ResourceVariant, expires time.Time) (string, error) {
		return s.signedResourceAccessURL(resource, variant, expires, options.Purpose == assets.PurposeProvider)
	})
	if err != nil {
		return nil, err
	}
	if options.DownloadFileName != "" && resource.Provider != "local" {
		switch access.Delivery {
		case assets.DeliveryCDN:
			access.URL, err = signedOSSObjectURL(setting, resource.ObjectKey, accessExpiry(access), options.DownloadFileName)
		case assets.DeliveryOrigin:
			access.URL, err = signedOriginObjectURL(setting, resource.ObjectKey, accessExpiry(access), options.DownloadFileName)
		}
		if err != nil {
			return nil, err
		}
...
func restoreLegacyCDNDelivery(setting ossSettingValue) ossSettingValue {
	if setting.CDNBaseURL == "" || setting.Delivery.CDNAuthMode != "" || setting.Delivery.RequireCDN {
		return setting
	}
	if setting.Provider == qiniuKodoProvider {
		setting.Delivery.CDNAuthMode = "qiniu"
	} else {
		setting.Delivery.CDNAuthMode = "public"
	}
	return setting
}

func (s *Service) PrepareResourceDelivery(userID, id string, options ResourceAccessOptions, rangeHeaders ...string) (*ResourceDelivery, error) {
...
func (s *Service) prepareResourceDelivery(userID string, resource *model.Resource, options ResourceAccessOptions, rangeHeader string) (*ResourceDelivery, error) {
	return s.prepareResourceDeliveryWithStream(userID, resource, options, rangeHeader, true)
}

func (s *Service) prepareResourceDeliveryWithoutStream(userID string, resource *model.Resource, options ResourceAccessOptions, rangeHeader string) (*ResourceDelivery, error) {
	return s.prepareResourceDeliveryWithStream(userID, resource, options, rangeHeader, false)
}

func (s *Service) prepareResourceDeliveryWithStream(userID string, resource *model.Resource, options ResourceAccessOptions, rangeHeader string, openStream bool) (*ResourceDelivery, error) {
	...
	if access.Delivery == assets.DeliveryCDN || access.Delivery == assets.DeliveryOrigin {
		delivery.RedirectURL = access.URL
	}
	if options.DescribeOnly || access.Delivery == assets.DeliveryCDN || access.Delivery == assets.DeliveryOrigin {
		if !options.DescribeOnly && access.Delivery == assets.DeliveryCDN {
			available, probeErr := s.redirectResourceAvailable(options.Context, resource, access.URL)
			if probeErr != nil {
...
func accessExpiry(access *ResourceAccess) time.Time {
