...
func TestHostChannelPluginsRecoverExistingVideoWithoutCreating(t *testing.T) {
	base, db := newTimelineTaskTestService(t)
	svc := NewWithRuntimeCapabilities(base.repo, t.TempDir(), RuntimeCapabilities{desktopLocalChannels: true})
	if svc.pluginRuntimeErr != nil {
		t.Fatal(svc.pluginRuntimeErr)
	}
	if svc.runtimeErr != nil {
		t.Fatal(svc.runtimeErr)
	}
	center := svc.pluginRuntime
	for _, tt := range hostChannelDispatchCases {
		if tt.mode != "video" {
			continue
		}
		t.Run(tt.providerID, func(t *testing.T) {
			...
			upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				if r.Method != http.MethodGet || r.URL.RequestURI() != tt.basePath+tt.pollPath {
					t.Errorf("recovery must only poll: %s %s", r.Method, r.URL.RequestURI())
					http.NotFound(w, r)
					return
				}
				polls.Add(1)
				...
				if tt.providerID == "weijin-video" || tt.providerID == "tianyue-video" {
					...
					_, _ = w.Write([]byte(`{"code":0,"data":{"id":"task-1","taskId":"task-1","status":"processing"}}`))
				}
			}))
			defer upstream.Close()
			raw, err := json.Marshal(canvasGenerationInput{
				Mode: "video", Prompt: "recovery test",
				Config: providerConfig{
					InterfaceType: tt.providerID, Model: tt.model, BaseURL: upstream.URL + tt.basePath,
					APIKey: "test-key", AllowLocalChannel: true,
				},
			})
			if err != nil {
				t.Fatal(err)
			...
			task := &model.Task{
				ID: newID(), UserID: "test-owner", Type: "canvas_video",
				Status: model.TaskStatusFailed, ProviderRequestID: "task-1", InputJSON: string(raw),
			}
			if err := db.Create(task).Error; err != nil {
				t.Fatal(err)
			}
			if _, err := svc.QueryFailedVideoTask(context.Background(), "another-user", task.ID); err == nil {
				t.Fatal("another account queried the task")
			}
			result, err := svc.QueryFailedVideoTask(context.Background(), task.UserID, task.ID)
			if err != nil {
				t.Fatal(err)
			}
			if result.Recovered || !result.PollingResumed || result.Task.Status != model.TaskStatusRunning || result.Task.ProviderRecoveryAt == nil || result.ProviderStatus != "processing" || polls.Load() != 1 {
				t.Fatalf("recovery result=%+v, polls=%d", result, polls.Load())
			}
			if _, err := svc.QueryFailedVideoTask(context.Background(), task.UserID, task.ID); err == nil {
				t.Fatal("repeated manual query accepted an already resumed task")
			}
			if err := db.Model(task).Update("status", model.TaskStatusFailed).Error; err != nil {
				t.Fatal(err)
			}
			if _, err := center.setEnabled(tt.pluginID, false); err != nil {
				t.Fatal(err)
			}
			if _, err := svc.QueryFailedVideoTask(context.Background(), task.UserID, task.ID); err == nil {
				t.Fatal("disabled plugin accepted recovery")
			}
			if polls.Load() != 1 {
				t.Fatal("disabled plugin sent a recovery request")
			}
			if _, err := center.setEnabled(tt.pluginID, true); err != nil {
				t.Fatal(err)
			}
		})
	}
