...
func runProtocolAdapterTask(ctx context.Context, input canvasGenerationInput, adapter protocol.Adapter) (map[string]interface{}, error) {
	policy := declarativeProtocolPollPolicy(input.Mode)
	return runProtocolAdapterTaskWithPolicy(ctx, input, adapter, protocolPollPolicyFor(input.Config.InterfaceType, policy))
}
...
func runProtocolAdapterTaskWithPolicy(ctx context.Context, input canvasGenerationInput, adapter protocol.Adapter, policy videoPollPolicy) (map[string]interface{}, error) {
	...
	if input.Config.InterfaceType == string(model.ChannelInterfaceWeijinVideo) && taskID == "" {
...
func prepareCustomProtocolRequest(input canvasGenerationInput) (protocol.GenerationRequest, error) {
	request := protocolRequestFromInput(input)
	...
	case string(model.ChannelInterfaceAIStarsLabImage), string(model.ChannelInterfaceAIStarsLabVideo):
		...
		if route == nil || strings.TrimSpace(route.Channel) == "" {
			return request, errors.New("AIStarsLab 模型缺少线路编码，请在后台重新拉取该渠道模型")
		}
		...
		if input.Config.InterfaceType == string(model.ChannelInterfaceAIStarsLabImage) {
			if input.Mask != nil {
				return request, errors.New("AIStarsLab 图片协议不支持蒙版编辑，请移除蒙版后重试")
			}
			if route.InputImagesMax >= 0 && len(request.Images) > route.InputImagesMax {
				return request, fmt.Errorf("AIStarsLab 当前模型最多支持 %d 张参考图，当前连接了 %d 张", route.InputImagesMax, len(request.Images))
			}
			quality, err := aiStarsLabRequestQuality(route, nil, request.Quality)
			if err != nil {
				return request, err
			}
			request.Quality = quality
			request.AspectRatio = aiStarsLabImageRatio(request.AspectRatio, route)
			return request, validateCustomProtocolMediaURLs(input.Config.InterfaceType, request)
		}
		if len(request.Audios) > 0 && len(request.Images) == 0 {
			return request, errors.New("AIStarsLab 参考音频必须同时提供至少 1 张参考图片")
		}
		if route.InputVideosMax >= 0 && len(request.Videos) > route.InputVideosMax {
			return request, fmt.Errorf("AIStarsLab 当前模型最多支持 %d 个参考视频，当前连接了 %d 个", route.InputVideosMax, len(request.Videos))
		}
		if route.InputAudiosMax >= 0 && len(request.Audios) > route.InputAudiosMax {
			return request, fmt.Errorf("AIStarsLab 当前模型最多支持 %d 个参考音频，当前连接了 %d 个", route.InputAudiosMax, len(request.Audios))
		}
		mode, err := aiStarsLabVideoMode(route, len(request.Images))
		if err != nil {
			return request, err
		}
		request.Extra["aistarslabMode"] = mode
		quality, err := aiStarsLabRequestQuality(route, input.VideoCapability, defaultString(request.Resolution, request.Quality))
		if err != nil {
			return request, err
		}
		request.Quality, request.Resolution = quality, quality
	case string(model.ChannelInterfaceHuiQuYunVideo):
		if len(request.Audios) > 0 && len(request.Images) == 0 && len(request.Videos) == 0 {
			return request, errors.New("汇取云参考音频必须搭配参考图片或参考视频")
		}
		request.Duration = huiQuYunRequestDuration(request.Model, request.Duration)
		if isHuiQuYun933MultipartVideoModel(request.Model) {
			request.AspectRatio = normalizeHuiQuYunMX933VideoRatio(request.AspectRatio)
			quality := strings.ToLower(strings.TrimSpace(request.Resolution))
			if quality != "480p" && quality != "720p" {
				quality = "720p"
			}
			request.Resolution = quality
		} else {
			request.AspectRatio = normalizeHuiQuYunVideoRatio(request.AspectRatio)
...
func huiQuYunRequestDuration(modelID string, requested int) int {
	if fixed := huiQuYunFixedVideoDuration(modelID); fixed > 0 {
		return fixed
	}
...
func validateCustomProtocolMediaURLs(interfaceType string, request protocol.GenerationRequest) error {
	if interfaceType == string(model.ChannelInterfaceHuiQuYunVideo) && isHuiQuYun933MultipartVideoModel(request.Model) {
		return nil
	}
	switch interfaceType {
	case string(model.ChannelInterfaceGlobalAiOpcImage),
		string(model.ChannelInterfaceGlobalAiOpcVideo),
	...
	for _, media := range append(append(append([]protocol.MediaReference{}, request.Images...), request.Videos...), request.Audios...) {
