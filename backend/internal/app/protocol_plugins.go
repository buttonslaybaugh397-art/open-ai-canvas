...
func (c *pluginRuntime) bootstrapBuiltInPlugins() error {
	...
	bundledManifests := append([]protocol.Manifest{}, protocol.BundledHostManifests()...)
	bundledManifests = append(bundledManifests, bundledWorkflowPluginManifests()...)
	bundledManifests = append(bundledManifests, bundledPaymentPluginManifests()...)
...
func (c *pluginRuntime) reload() error {
	...
	for id, record := range plugins {
		...
		if isBuiltInPluginSource(record.Source) && strings.HasPrefix(strings.TrimSpace(manifest.Runtime.Backend), "host:") {
			resolver = func(execution string) (protocol.Adapter, bool) {
