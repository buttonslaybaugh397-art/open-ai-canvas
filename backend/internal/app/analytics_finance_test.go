...
func TestAdminAnalyticsFinanceUsesSettledSnapshotsAndDeduplicates(t *testing.T) {
	...
	if err := db.AutoMigrate(&model.BillingOrder{}, &model.ApiCallLog{}, &model.Task{}, &model.UserDailyActivity{}, &model.User{}, &model.ModelChannel{}); err != nil {
		t.Fatal(err)
	}
	...
	if len(rows) != len(result.Users)+1 || len(rows[0]) != 43 || rows[0][1] != "用户ID" || rows[0][32] != "已结算消耗(微积分)" || rows[0][42] != "每日明细(JSON)" {
		t.Fatalf("user summary CSV contract changed: %v", rows)
	}
	users := make(map[string]AnalyticsUserRow, len(result.Users))
	for _, user := range result.Users {
		users[user.UserID] = user
	}
	for _, row := range rows[1:] {
		user, ok := users[row[1]]
		if !ok || row[19] != strconv.Itoa(user.Requests) || row[32] != strconv.FormatInt(user.CreditsConsumedMicrocredits, 10) {
			t.Fatalf("user CSV differs from consumption statistics: %v", row)
		}
		delete(users, row[1])
	}
	if len(users) != 0 || len(result.Users) != 2 || len(result.Channels) != 2 {
		t.Fatalf("finance must coexist with per-user and per-channel statistics: %+v", result)
	}
	query.Model = "paid"
