...
func TestMigrateSchemaUpgradesLegacyStableLineage(t *testing.T) {
	db, err := Open(Config{Driver: "sqlite", DSN: "file:legacy-stable-lineage?mode=memory&cache=shared"})
	if err != nil {
		t.Fatal(err)
	}
	sqlDB, err := db.DB()
	...
	for _, item := range legacyStableSchemaMigrations {
		if err := db.Create(&schemaMigration{Version: item.version, Name: item.name, Checksum: item.checksum, AppliedAt: time.Now().UTC()}).Error; err != nil {
			t.Fatal(err)
		}
	}
	if err := db.Create(&model.User{ID: "legacy-user", Username: "legacy-user", PasswordHash: "kept"}).Error; err != nil {
		t.Fatal(err)
	}
	...
	var user model.User
	if err := db.First(&user, "id = ?", "legacy-user").Error; err != nil || user.PasswordHash != "kept" {
		t.Fatalf("legacy user changed: %+v %v", user, err)
	}
	for _, expected := range schemaMigrations {
...
func TestMigrateSchemaRejectsTamperedLegacyStableLineage(t *testing.T) {
	db, err := Open(Config{Driver: "sqlite", DSN: "file:legacy-stable-tampered?mode=memory&cache=shared"})
	if err != nil {
		t.Fatal(err)
	}
	if err := db.AutoMigrate(&schemaMigration{}); err != nil {
	...
	for _, item := range legacyStableSchemaMigrations[:7] {
		checksum := item.checksum
		if item.version == 5 {
			checksum = "tampered"
		}
		if err := db.Create(&schemaMigration{Version: item.version, Name: item.name, Checksum: checksum, AppliedAt: time.Now().UTC()}).Error; err != nil {
			t.Fatal(err)
		}
	}
