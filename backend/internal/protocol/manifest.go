...
func LoadInstalledProviders(data []byte, resolve AdapterResolver) ([]Adapter, error) {
	...
	if strings.HasPrefix(strings.TrimSpace(manifest.Runtime.Backend), "host:") {
		...
		adapters := make([]Adapter, 0, len(manifest.Contributes.Providers))
		for index, provider := range manifest.Contributes.Providers {
			execution := name
			if name == "providers" {
				execution = provider.ID
			} else if len(manifest.Contributes.Providers) != 1 {
				return nil, fmt.Errorf("host execution %q cannot bind multiple providers", name)
			}
			...
			projection := manifest
			if err := normalizeManifestForProvider(&projection, index); err != nil {
				return nil, err
			}
			adapters = append(adapters, metadataAdapter{metadata: projection.Metadata, delegate: adapter})
		}
		return adapters, nil
	}
	if len(manifest.Contributes.Providers) == 0 {
