...
- ` + durationRule + `
- ` + countRule + `
- 当前剧本中的对白必须逐字保留，不改词、不删词、不交换说话人；对白跨镜时按连续且互斥的片段承载，不能重复整句或漏掉后半句。
- 没有用户硬时长时，按自然可懂的播读、停顿和动作完成时间设计；常态中文对白约按每秒 4 个有效字估算，明确急促可按 5、明确沉重或迟疑可按 3。该规则只做可读性校验，不按字数机械切镜。
- 对白与真实同步的动作取两者所需时间的较大值；先后动作按时间轴排列，不得为了填满时长强行并行或拖长短对白。
- camera、motion、videoPrompt 必须能执行：说明观察起点、触发或叙事动机、运动路径或焦点交接和停止/结束状态；不得用“节奏需要”“丰富景别”等空泛理由代替摄影设计。
- 成品提示词字段只写观众可见的画面、动作、表演、视点变化、声音和结束状态；duration_basis、segment_reason、cut_motivation、审片结果、分组理由、推理过程和 JSON 协议标记属于内部信息，禁止写入 visualPrompt、videoPrompt、camera、motion、timeBeats 或其他成品字段。
- 角色、场景、道具、动作、称谓和对白只能来自当前剧情与已确认资产；不得从示例、历史稿件、技能正文或审查建议补充新事实。
