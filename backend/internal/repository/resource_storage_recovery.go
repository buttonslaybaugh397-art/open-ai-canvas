...
func (r *Repository) ClaimNextResourceCloudBackupCleanup(owner string, leaseDuration time.Duration) (*model.Resource, error) {
	var resource model.Resource
	now := time.Now()
	err := r.db.Transaction(func(tx *gorm.DB) error {
		available := "provider <> ? AND local_backup_key <> '' AND cloud_sync_status = ? AND (cloud_sync_next_attempt_at IS NULL OR cloud_sync_next_attempt_at <= ?) AND (cloud_sync_lease_expires_at IS NULL OR cloud_sync_lease_expires_at <= ?)"
		query := tx.Where(available, "local", model.ResourceCloudSyncStatusSynced, now, now).
			Where("kind = ?", "video").
			Where("status = ?", model.ResourceStatusReady).Order("cloud_sync_next_attempt_at asc, updated_at asc").Limit(1)
		if r.Dialect() == "postgres" {
			query = query.Clauses(clause.Locking{Strength: "UPDATE", Options: "SKIP LOCKED"})
		...
		claim := tx.Model(&model.Resource{}).Where("id = ?", resource.ID)
		if r.Dialect() != "postgres" {
			claim = claim.Where(available, "local", model.ResourceCloudSyncStatusSynced, now, now).Where("kind = ?", "video")
		}
		updated := claim.Updates(map[string]any{
			"cloud_sync_lease_owner":      owner,
			"cloud_sync_lease_expires_at": now.Add(leaseDuration),
...
func (r *Repository) CompleteResourceCloudBackupCleanup(id string, owner string) error {
	return r.db.Model(&model.Resource{}).Where("id = ? AND cloud_sync_status = ? AND cloud_sync_lease_owner = ?", id, model.ResourceCloudSyncStatusSynced, owner).Updates(map[string]any{
		"local_backup_key":            "",
		"cloud_sync_next_attempt_at":  nil,
		"cloud_sync_lease_owner":      "",
		"cloud_sync_lease_expires_at": nil,
...
func (r *Repository) RetryResourceCloudBackupCleanup(id string, owner string, lastError string, nextAttemptAt time.Time) error {
	return r.db.Model(&model.Resource{}).Where("id = ? AND cloud_sync_status = ? AND cloud_sync_lease_owner = ?", id, model.ResourceCloudSyncStatusSynced, owner).Updates(map[string]any{
