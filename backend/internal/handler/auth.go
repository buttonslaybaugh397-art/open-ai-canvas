...
func RegisterAdminRoutes(r *gin.RouterGroup, svc *service.Service) {
	...
	r.PATCH("/admin/users/:id", func(c *gin.Context) {
		...
		c.Header("Cache-Control", "no-store")
		if req.Password != "" && updated.ID == user.ID {
			clearSessionCookie(c)
		}
	...
	r.GET("/admin/settings/oss/credentials", func(c *gin.Context) {
		...
		if !enforceRateLimit(c, "admin-storage-credentials:"+user.ID, 6, time.Minute) {
			return
		}
		credentials, err := svc.AdminOSSCredentials(user)
		if err != nil {
			failService(c, err)
			return
		}
		ok(c, gin.H{"credentials": credentials})
	})
