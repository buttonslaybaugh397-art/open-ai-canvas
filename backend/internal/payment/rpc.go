...
func NewRPCProvider(descriptor Descriptor, packageDir, entry string) (*RPCProvider, error) {
	...
	normalizedEntry := filepath.ToSlash(filepath.Clean(filepath.FromSlash(strings.TrimSpace(entry))))
	if filepath.IsAbs(filepath.FromSlash(normalizedEntry)) || normalizedEntry == "backend" || !strings.HasPrefix(normalizedEntry, "backend/") {
	...
	return &RPCProvider{descriptor: descriptor, command: filepath.Join(packageDir, filepath.FromSlash(normalizedEntry)), dir: packageDir}, nil
}
...
func (p *RPCProvider) call(ctx context.Context, request rpcRequest, output any) error {
	...
	if filepath.IsAbs(command) || strings.ContainsAny(command, "/\\") {
		...
		if !info.Mode().IsRegular() {
			return p.startError(fmt.Errorf("payment plugin executable is not a regular file: %w", syscall.EACCES))
		}
