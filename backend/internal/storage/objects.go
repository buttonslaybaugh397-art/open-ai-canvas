...
var (
	ErrObjectMissing = errors.New("资源对象不存在")
	ErrCDNTemporary  = errors.New("对象存储 CDN 临时不可用")
)
...
func getCDNObjectRange(setting Settings, objectKey string, rangeHeader string) (*ObjectStream, error) {
	...
	if setting.Delivery.CDNAuthMode == "" && setting.Provider != qiniuKodoProvider {
...
func SignedOSSObjectURL(setting Settings, objectKey string, expiresAt time.Time, downloadFileName ...string) (string, error) {
	setting = NormalizeSettings(setting)
	...
	return SignedOriginObjectURL(setting, objectKey, expiresAt, downloadFileName...)
}

func SignedOriginObjectURL(setting Settings, objectKey string, expiresAt time.Time, downloadFileName ...string) (string, error) {
	setting = NormalizeSettings(setting)
	if setting.Provider == s3Provider {
		return SignedS3ObjectURL(setting, objectKey, expiresAt, downloadFileName...)
	}
	if setting.Provider == qiniuKodoProvider {
		return SignedQiniuS3ObjectURL(setting, objectKey, expiresAt, downloadFileName...)
	}
	if setting.Provider == tencentCOSProvider {
...
func SignedAliyunOSSObjectURL(setting Settings, objectKey string, expiresAt time.Time, downloadFileName ...string) (string, error) {
...
func SignedCOSObjectURL(setting Settings, objectKey string, expiresAt time.Time, downloadFileName ...string) (string, error) {
	if strings.TrimSpace(setting.AccessKeyID) == "" || strings.TrimSpace(setting.AccessKeySecret) == "" {
	...
	options := &cos.ObjectGetOptions{ResponseContentDisposition: resourceDownloadDisposition(downloadFileName...)}
...
func SignedQiniuObjectURL(setting Settings, objectKey string, expiresAt time.Time, downloadFileName ...string) (string, error) {
	if setting.AccessKeyID == "" || setting.AccessKeySecret == "" {
		return "", errors.New("七牛云 Kodo 访问密钥不可用")
	}
	objectKey = strings.TrimLeft(strings.TrimSpace(objectKey), "/")
	...
	if name := resourceDownloadFileName(downloadFileName...); name != "" {
...
func SignedQiniuS3ObjectURL(setting Settings, objectKey string, expiresAt time.Time, downloadFileName ...string) (string, error) {
...
func OssCDNObjectURL(raw string, objectKey string, downloadFileName ...string) (string, error) {
...
func resourceDownloadFileName(fileNames ...string) string {
	if len(fileNames) == 0 || strings.TrimSpace(fileNames[0]) == "" {
		return ""
	}
	name := path.Base(strings.ReplaceAll(fileNames[0], "\\", "/"))
...
func resourceDownloadDisposition(fileNames ...string) string {
	if name := resourceDownloadFileName(fileNames...); name != "" {
